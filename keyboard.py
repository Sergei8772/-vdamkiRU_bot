...
def create_board_markup(game: CheckersGame, game_id: str = None, is_bot_game: bool = True) -> InlineKeyboardMarkup:
    ...
    for row in range(8):
        ...
        for col in range(8):
            ...
            callback_data = f"move:{row}:{col}" if is_bot_game else f"friend_move:{row}:{col}"
            if game_id:
    ...
    if game.game_active:
        if is_bot_game:
            ...
            control_buttons = [
                InlineKeyboardButton("🤝 Ничья", callback_data=f"draw:{game_id}"),
                InlineKeyboardButton("🏳️ Сдаться", callback_data=f"friend_surrender:{game_id}"),
                InlineKeyboardButton("📋 Меню", callback_data="main_menu")
            ]
    else:
        control_buttons = [
